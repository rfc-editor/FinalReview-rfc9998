...
docname: draft-iab-agews-report-03
stream: IAB
number: 9998
date: 2026-06
consensus: true
...
informative:
  ...
  HANSON:
    ...
    date: 2025-10
    refcontent: IAB/W3C Workshop on Age-Based Restrictions on Content Access
  FATE:
    title: "Face Analysis Technology Evaluation (FATE) Part 10: Performance of Passive, Software-Based Presentation Attack Detection (PAD) Algorithms"
...
Regulators and legislators around the world are increasingly restricting what can be made available to young people on the Internet in an effort to reduce the online harm that they encounter.
...
While this workshop did not provide specific standards proposals or take positions on the advisability of regulatory proposals, it was suggested that leadership bodies, including the IAB and the Technical Architecture Group (TAG), could be places to make such statements.
...
Architectures effective for the goals and less likely to have profound harmful consequences may require the cooperation of multiple actors fulfilling different roles (see {{roles}}). To that end, standardization may be especially important for interoperable, collaborative development of architectures involving both servers and clients.
...
Relating these functions to the roles described in {{roles}}, all age assurance types fit the "verifier" role, whereas age gating applies to the "enforcer" role.
...
Throughout this discussion, it was acknowledged that active deployments tend to fall into a common pattern, where content providers are required to age-gate access and contract a third party to interpose that service. Several participants noted that this is a somewhat natural consequence of some of the constraints that actors are subject to.  {{f-typical}} shows the typical deployment model for age-gated content and services, along with the roles from {{roles}}.
...
The following subsections cover each of the main topics of discussion sessions.
